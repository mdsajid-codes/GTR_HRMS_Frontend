...
const WorkType = ({ embedded = false }) => {
    ...
    const [syncingWorkType, setSyncingWorkType] = useState(null);
    ...
    const handleOpenSyncModal = (workType) => {
        setSyncingWorkType(workType);
    };

    const handleConfirmSync = async (workType, selectedEmployeeCodes) => {
        ...
        try {
            ...
            for (const employeeCode of selectedEmployeeCodes) {
                try {
                    const payload = { workTypeId: workType.id };
                    await axios.put(`${BASE_URL}/time-attendence/${employeeCode}`, payload, { headers: { "Authorization": `Bearer ${token}` } });
                    successCount++;
                } catch (updateErr) {
                    console.error(`Failed to update work type for ${employeeCode}:`, updateErr);
                    errorCount++;
                }
            }
    ...
    const content = (
        <>
                                            ...
                                            <tr key={wt.id} className="border-b border-slate-200 hover:bg-slate-50">
                                                ...
                                                <td className="td-cell">
                                                    <div className="flex items-center gap-2"> 
                                                        <button onClick={() => handleOpenSyncModal(wt)} className="p-2 text-slate-500 hover:text-green-600 hover:bg-green-100 rounded-full" title="Sync to employees" disabled={!!syncingWorkType}>
                                                            {syncingWorkType?.id === wt.id
                                                                ? <Loader className="h-4 w-4 animate-spin" />
                                                                : <RefreshCw className="h-4 w-4" />}
                                                        </button>
                                                        <button onClick={() => handleEdit(wt)} className="p-2 text-slate-500 hover:text-blue-600 hover:bg-blue-100 rounded-full" title="Edit" disabled={!!syncingWorkType}>
                                                            <Edit className="h-4 w-4" />
                                                        </button>
                                                        <button onClick={() => handleDelete(wt.id, wt.name)} className="p-2 text-slate-500 hover:text-red-600 hover:bg-red-100 rounded-full" title="Delete" disabled={!!syncingWorkType}>
            ...
            />
            <SyncPolicyModal 
                isOpen={!!syncingWorkType} 
                onClose={() => setSyncingWorkType(null)} 
                policy={syncingWorkType}
