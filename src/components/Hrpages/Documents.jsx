                ...
                <form onSubmit={handleSubmit}>
                    <div className="p-6 space-y-4">
                        <div>
                            ...
                            <input id="file" type="file" onChange={handleFileChange} required className="input" />
                        </div>
                        <div>
                            <label htmlFor="docType" className="block text-sm font-medium text-slate-700">Document Type</label>
                            <select id="docType" value={docType} onChange={(e) => setDocType(e.target.value)} className="input">
                                {DOC_TYPES.map(type => <option key={type} value={type}>{type}</option>)}
                            </select>
                        </div>
                        <div>
                            <label htmlFor="remarks" className="block text-sm font-medium text-slate-700">Remarks</label>
                            <textarea id="remarks" value={remarks} onChange={(e) => setRemarks(e.target.value)} rows="3" className="input" />
                        </div>
...
const EditDocumentModal = ({ isOpen, onClose, onSave, document, loading }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-md">
                <div className="p-4 border-b flex justify-between items-center">
                    <h2 className="text-xl font-semibold">Edit Document Details</h2>
                    <button onClick={onClose} className="p-2 rounded-full hover:bg-slate-100"><X className="h-5 w-5" /></button>
                </div>
                <form onSubmit={handleSubmit}>
                    <div className="p-6 space-y-4">
                        <div>
                            <label htmlFor="edit-docType" className="block text-sm font-medium text-slate-700">Document Type</label>
                            <select id="edit-docType" name="docType" value={formData.docType} onChange={handleChange} className="input">
                                {DOC_TYPES.map(type => <option key={type} value={type}>{type}</option>)}
                            </select>
                        </div>
                        <div>
                            <label htmlFor="edit-remarks" className="block text-sm font-medium text-slate-700">Remarks</label>
                            <textarea id="edit-remarks" name="remarks" value={formData.remarks} onChange={handleChange} rows="3" className="input" />
                        </div>
                        <div>
                            <label className="inline-flex items-center">
                                <input type="checkbox" name="verified" checked={formData.verified} onChange={handleChange} className="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500" />
                                <span className="ml-2 text-sm text-slate-700">Mark as Verified</span>
                            </label>
                        </div>
                    </div>
                    <div className="p-4 border-t bg-slate-50 flex justify-end gap-2">
                                ...
                                {documents.length > 0 ? (
                                    documents.map(doc => (
                                        <tr key={doc.id} className="border-b border-slate-200 hover:bg-slate-50">
                                            <td className="td-cell font-medium">{doc.docType}</td>
                                            <td className="td-cell text-sm text-slate-500">{doc.fileName}</td>
                                            <td className="td-cell">
                                                {doc.verified ? <CheckCircle className="h-5 w-5 text-green-600" /> : <XCircle className="h-5 w-5 text-slate-400" />}
                                            </td>
                                            <td className="td-cell">
                                                <div className="flex items-center gap-1">
                                                    <button onClick={() => handleView(doc.id)} className="p-2 text-slate-500 hover:text-blue-600 rounded-full" title="View"><Eye className="h-4 w-4" /></button>
                                                    <button onClick={() => handleDownload(doc.id)} className="p-2 text-slate-500 hover:text-blue-600 rounded-full" title="Download"><Download className="h-4 w-4" /></button>
                                                    <button onClick={() => { setEditingDocument(doc); setIsEditModalOpen(true); }} className="p-2 text-slate-500 hover:text-blue-600 rounded-full" title="Edit"><Edit className="h-4 w-4" /></button>
                                                    <button onClick={() => handleDelete(doc.id)} className="p-2 text-slate-500 hover:text-red-600 rounded-full" title="Delete"><Trash2 className="h-4 w-4" /></button>
                                                </div>
                                            </td>
                                        </tr>
                                    ))
