...
import { ClipboardCheck, Edit, Trash2, PlusCircle, Loader, X, Search } from 'lucide-react';
...
const TaskForm = ({ item, onSave, onCancel, locations }) => {
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-5">
            <FormField label="Task Name" name="name" value={formData.name} onChange={handleChange} required placeholder="e.g., Quality Check, Packaging" />
            <FormField label="Description" name="description" value={formData.description} onChange={handleChange} placeholder="Brief description of this task" />
            <div className="space-y-1">
...
// Main component for Task CRUD
const Task = ({ locationId }) => {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            const [itemsRes, locationsRes] = await Promise.all([
                axios.get(`${API_URL}/production/tasks`, { headers: authHeaders }),
    ...
    const handleSave = async (itemData) => {
        ...
        const url = isUpdating ? `${API_URL}/production/tasks/${itemData.id}` : `${API_URL}/production/tasks`;
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Are you sure you want to delete this task?')) {
            try {
                await axios.delete(`${API_URL}/production/tasks/${id}`, { headers: authHeaders });
                fetchData();
            } catch (err) {
    ...
    return (
        <>
            <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-semibold text-foreground">Manage Tasks</h3>
                <div className="flex items-center gap-2">
                    ...
                    <button onClick={handleAdd} className="flex items-center gap-2 btn-secondary"><PlusCircle size={16} /> Add Task</button>
                </div>
            </div>
            ...
            <Modal isOpen={isModalOpen} onClose={handleCloseModal} title={currentItem?.id ? 'Edit Task' : 'Add Task'}>
                <TaskForm item={currentItem} onSave={handleSave} onCancel={handleCloseModal} locations={locations} />
            </Modal>
        </>
    );
};
