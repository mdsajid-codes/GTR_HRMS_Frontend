...
const IndustryForm = ({ item, onSave, onCancel, loading, locations }) => {
    const [formData, setFormData] = useState({ name: '', locationId: '' });

    useEffect(() => {
        if (item) {
            setFormData({ name: item.name || '', locationId: item.locationId || '' });
        } else {
...
const CrmIndustry = ({ locationId }) => {
    ...
    const filteredData = useMemo(() => {
        ...
        if (locationId === 'none') {
            filtered = industries.filter(item => !item.locationId);
        } else if (locationId && locationId !== 'all') {
            filtered = industries.filter(item => String(item.locationId) === String(locationId));
        }
        return filtered.filter(item => !searchTerm || item.name.toLowerCase().includes(searchTerm.toLowerCase()));
    }, [industries, searchTerm, locationId]);
    ...
    return (
        <div className="p-6 bg-card rounded-xl shadow-sm">
                ...
                <table className="min-w-full divide-y divide-border">
                    ...
                    <tbody className="bg-card divide-y divide-border text-foreground-muted">
                        {loading ? (
                            ...
                            <tr><td colSpan="4" className="text-center py-10"><AlertCircle className="mx-auto h-12 w-12 text-foreground-muted/50" /><h3 className="mt-2 text-sm font-medium text-foreground">No industries found</h3><p className="mt-1 text-sm">Get started by adding a new industry.</p></td></tr>
                        )}
                    </tbody>
                </table>
            </div>
            ...
            <Modal isOpen={isModalOpen} onClose={handleCloseModal} title={editingItem ? 'Edit Industry' : 'Add Industry'}>
                <IndustryForm item={editingItem} onSave={handleSave} onCancel={handleCloseModal} loading={modalLoading} locations={locations} />
            </Modal>
        </div>
    );
}
