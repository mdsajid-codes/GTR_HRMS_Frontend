...
const IndustryForm = ({ item, onSave, onCancel, loading }) => {
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
            <div>
                <label htmlFor="name" className="block text-sm font-medium text-foreground-muted">Industry Name</label>
                <input
                    id="name"
                    name="name"
                    value={name}
...
const CrmIndustry = () => {
    ...
    const fetchIndustries = useCallback(async () => {
        ...
        setError('');
        try {
            const response = await axios.get(`${API_URL}/settings/industries`, { headers: authHeaders });
    ...
    const handleSave = async (itemData) => {
        ...
        const url = isUpdating ? `${API_URL}/settings/industries/${itemData.id}` : `${API_URL}/settings/industries`;
        ...
        try {
            ...
            alert(`Error: ${err.response?.data?.message || 'Failed to save industry.'}`);
        } finally {
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Are you sure you want to delete this industry?')) {
            try {
                await axios.delete(`${API_URL}/settings/industries/${id}`, { headers: authHeaders });
                await fetchIndustries();
            } catch (err) {
                alert(`Error: ${err.response?.data?.message || 'Failed to delete industry.'}`);
            }
        }
    };
    ...
    const filteredData = useMemo(() => {
        ...
        return industries.filter(item => item.name.toLowerCase().includes(searchTerm.toLowerCase()));
    }, [industries, searchTerm]);

    return (
        <div className="p-6 bg-card rounded-xl shadow-sm">
            <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-semibold text-foreground">Manage Industries</h3>
                <div className="flex items-center gap-2">
                    ...
                    <button onClick={handleAdd} className="flex items-center gap-2 btn-secondary"><PlusCircle size={16} /> Add Industry</button>
                </div>
            </div>
            ...
            <div className="overflow-x-auto border border-border rounded-lg">
                <table className="min-w-full divide-y divide-border">
                    ...
                    <tbody className="bg-card divide-y divide-border text-foreground-muted">
                        {loading ? (
                            <tr><td colSpan="3" className="text-center py-10"><Loader className="animate-spin h-8 w-8 text-primary mx-auto" /></td></tr>
                            ...
                            <tr><td colSpan="3" className="text-center py-10"><AlertCircle className="mx-auto h-12 w-12 text-foreground-muted/50" /><h3 className="mt-2 text-sm font-medium text-foreground">No industries found</h3><p className="mt-1 text-sm">Get started by adding a new industry.</p></td></tr>
                        )}
                    </tbody>
                </table>
            </div>

            <Modal isOpen={isModalOpen} onClose={handleCloseModal} title={editingItem ? 'Edit Industry' : 'Add Industry'}>
                <IndustryForm item={editingItem} onSave={handleSave} onCancel={handleCloseModal} loading={modalLoading} />
            </Modal>
        </div>
    );
}
