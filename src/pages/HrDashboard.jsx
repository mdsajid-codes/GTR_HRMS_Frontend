...
import { Users, CalendarClock, Briefcase, UserPlus, ArrowUpRight, ArrowDownRight, CheckCircle, XCircle, PartyPopper, Eye, Search, UploadCloud, MapPin, Loader, UserCheck, UserX, Plane } from 'lucide-react';
...
const StatCard = ({ icon: Icon, title, value, change, changeType, color = 'blue', onClick }) => (
    <div onClick={onClick} className={`bg-white p-6 rounded-xl shadow-sm flex items-start justify-between ${onClick ? 'cursor-pointer hover:shadow-md hover:border-slate-300 border border-transparent' : ''}`}>
        <div>
...
const HrDashboard = () => {
    ...
    const fetchDashboardData = async () => {
        ...
        try {
            ...
            const [employeesRes, leavesRes, locationsRes, attendanceRes] = await Promise.all([
    ...
    const stats = dashboardData ? [
        ...
        { title: 'Pending Leaves', value: dashboardData.pendingLeaves, icon: CalendarClock, color: 'yellow', onClick: () => navigate('/leave') },
        ...
        <DashboardLayout>
            <div className="p-6 md:p-8">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-8">
                    {/* Today's Attendance */}
                    <div className="lg:col-span-2 bg-white p-6 rounded-xl shadow-sm h-full">
                        <h2 className="text-xl font-semibold text-slate-800 mb-4">Today's Attendance Summary</h2>
                        ...
                        <div className="mt-6">
                            <h3 className="text-lg font-medium text-slate-700 mb-3">Recent Check-ins</h3>
                                ...
                                {dashboardData?.todaysAttendance.filter(a => a.status === 'PRESENT' || a.status === 'HALF_DAY').map(att => (
                                    <div key={att.id} className="flex justify-between items-center p-3 bg-slate-50 rounded-lg">
                                        <div>
                                            ...
                                            <p className="text-xs text-slate-500">{att.employeeCode}</p>
                                        </div>
